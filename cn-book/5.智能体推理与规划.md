...
从这个示例可以看出，Agent 的系统提示明确要求它按照 ReAct 的思维模式工作：先分析问题，再根据需要调用工具执行操作（Act），随后观察工具返回的结果（Observe），最后继续推理，并决定是否需要再次调用工具。
...
```
**注释**:
- **1** 创建 ToT 生成 Agent，负责生成候选思路（Thought）。  
...
Reflexion（反思）是一种自我批判（Self-Critique）推理模式。在这种模式下，智能体会先评估自己的输出，分析哪里出了问题，然后生成反馈，并结合这些反馈再次尝试解决问题（见图 5.5）。反馈内容可以是一个提示（Hint）、错误分析（Error Analysis），或者针对下一次尝试的具体建议。多次尝试之后之所以能够得到更好的结果，并不是因为模型真正学会了什么，而是因为模型在新的上下文中加入了这些反馈信息，从而能够生成更好的回答，而不是因为模型参数发生了更新。
...
- **#1** Agent 的 Instructions 在运行时动态生成，因此可以把上下文（Context）动态注入到提示词中。
- **#2** 创建一个 Critic Agent，用来评估 Solver 的答案，并生成反馈意见。
- **#3** 按照设定的最大次数不断尝试寻找正确答案。
- **#4** Critic 生成的反馈（Hint）会作为 Context 注入到下一轮 Solver 中，实现反思式迭代。
...
推理（Reasoning）和规划（Planning）构成了构建高质量 Agent 的第二大核心支柱。在此前的章节中，我们已经介绍了第一项核心能力——**行动（Actions）与自主性（Agency）**。下一章将继续介绍第三项关键能力：**记忆（Memory）与知识（Knowledge）**，进一步完善 Agent 的整体能力体系。
...
3. 修改 Agent 提示词，使其遵循 ReAct 循环：
   ...
   **思考（Think）→（如有需要）调用工具（Call Tool）→观察结果（Observe）→继续思考（Think）。**
